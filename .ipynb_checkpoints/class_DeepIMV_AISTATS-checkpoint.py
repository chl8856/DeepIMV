...
'''
    After finalizing the results, further remove the following components:
        - view-specific IB
        - view-specific or view-shared predictions
        - view-mixture co-regularization component
        - mixture of expert 
        - and the corresponding coefficients and indicators
'''
...
### DEFINE STOCHASTIC ENCODER
def decoder(x_, o_dim_, num_layers_=1, h_dim_=100, activation_fn=tf.nn.relu, keep_prob_=1.0, w_reg_=None):
    '''
        INPUT
            x_            : (2D-tensor) input
            o_dim_        : (int) output dimension
        ...
        OUTPUT
            [None, 2xo_dim] tensor (with None activation)
    '''
    if num_layers_ == 1:
        out =  FC_Net(inputs=x_, num_outputs=o_dim_, activation_fn=None, weights_regularizer=w_reg_, scope='out')
    else: #num_layers > 1
...
### DEFINE ENCODER (for baseline)
def encoder(x_, o_dim_, num_layers_=1, h_dim_=100, activation_fn=tf.nn.relu, keep_prob_=1.0, w_reg_=None):
    '''
        INPUT
            x_            : (2D-tensor) input
            o_dim_        : (int) output dimension
...
def mixture_of_experts(mask_, mu_set_, logvar_set_):
    tmp = 0.
    for m in range(len(mu_set_)):
        tmp += tf.reshape(mask_[:, m], [-1,1]) * mu_set_[m]
    moe_mu = div(tmp, tf.reduce_sum(mask_, axis=-1, keepdims=True))
    
    tmp = 0.
    for m in range(len(mu_set_)):
        tmp += tf.reshape(mask_[:, m], [-1,1]) * tf.exp(logvar_set_[m])
        
    moe_var = div(tmp, tf.reduce_sum(mask_, axis=-1, keepdims=True)**2)
    moe_logvar = log(moe_var)
    
    return moe_mu, moe_logvar
...
class DeepIMV_AISTATS:
    ...
    def __init__(self, sess, name, input_dims, network_settings):
        ...
        self.mixture_mode     = network_settings['mixture_mode'] #poe vs moe      
...
#         with tf.name_scope(self.name):
        with tf.variable_scope(self.name):
            ...
            if self.mixture_mode == 'poe':
                self.mu_z, self.logvar_z = product_of_experts(self.mask, self.mu_z_set, self.logvar_z_set)                
            elif self.mixture_mode == 'moe':
                self.mu_z, self.logvar_z = mixture_of_experts(self.mask, self.mu_z_set, self.logvar_z_set)
            else:
                raise ValueError('Wrong Mixture Type: {poe, moe}')
...
#                self.LOSS_CONSISTENCY += 1./self.M * tf.reduce_mean(self.mask[:, m] * tf.reduce_sum(ds.kl_divergence(qz, qz_set[m]), axis=-1))  
#                self.LOSS_CONSISTENCY += 1./self.M * tf.reduce_mean(self.mask[:, m] * tf.reduce_sum((self.z - self.z_set[m])**2, axis=-1))
            ...
            #gradient         = opt.compute_gradients(self.LOSS_TOTAL, enc_vars+pred_vars)
            #self.solver      = opt.apply_gradients(grads_and_vars=gradient)
            
            
            ma = tf.train.ExponentialMovingAverage(0.999, zero_debias=True)
            ma_update = ma.apply(tf.model_variables())
            # ma_update = ma.apply(enc_vars+pred_vars)
